...
export const LaunchTouchpointButton: FC<LaunchTouchpointButtonProps> = ({
  ...
  const effectiveDescription =
    description || `Click to see this feature in action with Touchpoint.`; // Default or provided description
  ...
  const handleLaunch = () => {
    if (instance && conversationHandler) {
      ...
      if (intentName) {
        console.log(
          `Sending intent "${intentName}" with context for ${pageTitle}:`,
          context,
        );
        conversationHandler.sendIntent(intentName, context);
      } else {
        console.log(
          `Sending welcome intent with context for ${pageTitle}:`,
          context,
        );
    ...
    <div style={{ marginTop: "2rem", marginBottom: "2rem" }}>
      ...
      <button
        ...
        style={{
          ...
          backgroundColor: isTouchpointReady
            ? "var(--accent)"
            : "var(--primary-10)",
          color: isTouchpointReady
            ? "var(--secondary-80)"
            : "var(--primary-40)",
          border: "none",
        ...
        title={
          isTouchpointReady ? effectiveButtonLabel : "Touchpoint is not ready"
        }
      >
        <span style={{ width: "1.2rem", height: "1.2rem" }}>
          <TouchpointIcon />
        </span>
      ...
      {!isTouchpointReady && (
        ...
        >
          Touchpoint is initializing or not available. Please wait or check
          console for errors.
        </p>
      )}
    </div>
